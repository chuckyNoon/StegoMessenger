...
// TODO: refactor and get rid of Android dependencies
class LsbAlgorithm @Inject constructor() : StegoAlgorithm {

    override fun decodeBitmap(containerBitmap: Bitmap): HiddenContent? {
    ...
    override fun encodeBitmap(containerBitmap: Bitmap, hiddenContent: HiddenContent): Bitmap? {
        val containerHeight = containerBitmap.height
        val containerWidth = containerBitmap.width
        ...
        when (hiddenContent) {
            is HiddenContent.Image -> {
                startLabel = LABEL_START_IMG
                endLabel = LABEL_END_IMG
            }
            is HiddenContent.Text -> {
                startLabel = LABEL_START_TEXT
                endLabel = LABEL_END_TEXT
            }
        }

        val dataByteArray = hiddenContent.toByteArray()
    ...
    private fun tryToParseImage(messageStringBuilder: StringBuilder): HiddenContent.Image? {
    ...
    private fun tryToParseText(messageStringBuilder: StringBuilder): HiddenContent.Text? {
        val hiddenStartIndex =
    ...
    companion object {
        ...
        private const val LABEL_START_IMG = "e1wfw"
        private const val LABEL_END_IMG = "erver32c"

        private const val LABEL_START_TEXT = "rew3434czx"
        private const val LABEL_END_TEXT = "34mf3.f/f34"
    }
}
